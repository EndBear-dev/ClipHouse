# Privacy policy for Chrome extensions developed by Endbear
## Chrome拡張のプライバシーポリシー

本プライバシーポリシーは、Endbear（以下、「当開発者」）が開発したGoogleChromeの拡張機能(Extension)(以下、「拡張機能」とします。)の利用において、利用者の個人情報もしくはそれに準ずる情報を取り扱う際に、当開発者が遵守する方針を示したものです。
...
#### 取得方法
利用者の拡張機能実行時に、閲覧中のサイト情報を取得します。
取得された内容は、ブラウザ内のLocalStorageに保存されます。
また、拡張機能内に閲覧履歴の取得ボタンを表示します。
取得されたデータは、利用者が指定したデータにより操作したのち、ブラウザに再格納されます。
拡張機能が閲覧履歴のデータを保存することはいたしません。
...
これにより、利用者の登録データが拡張機能に保持するため手間を省くことが可能になり、利便性が向上します。
